...
const WalletConnectAccountSelection: React.FC<Props> = () => {
  ...
  const onWalletChange = async () => {
    try {
      ...
      if (selectedWallet) {
        const coins = await coinDb.getAll({ walletId: selectedWallet._id });
        setCoinData(
          coins
            .filter(e => COINS[e.slug].group === CoinGroup.Ethereum)
            .map(e => {
              const coin = COINS[e.slug];
