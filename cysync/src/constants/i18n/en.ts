...
const en: I18nStrings = {
  ERRORS: {
    ...
    DEVICE_NOT_CONNECTED:
      'Connect the cypherock X1 device before proceeding with this process',
    DEVICE_DISCONNECTED_IN_FLOW:
      'Hardware Wallet disconnected. Reconnect the Cypherock X1 device',
    DEVICE_TIMEOUT_ERROR:
      'Some internal error occurred\n This maybe due to inactivity on the Cypherock X1 device',
    DEVICE_NOT_READY:
      'Bring the device to the main menu before starting any process.',
    DEVICE_NOT_READY_IN_INITIAL:
      'Error connecting the device, reconnect the device and try again.',
    DEVICE_NOT_SUPPORTED:
      'The connected device is not supported, make sure that CySync and X1 device are up to date.',
    // I dont understand how this message would be helpful.
    DEVICE_MISCONFIGURED:
      'Your device is misconfigured. Upgrade your device. If the problem persists, contact us.',
    DEVICE_INFO_UNKNOWN_ERROR: `Some interal error occured. Disconnect and then reconnect the device.`,
    DEVICE_NOT_IN_UPDATE_RECEIVING_MODE: `Some interal error occured. Disconnect and then reconnect the device.`,
    ...
    WALLET_PARTIAL_STATE:
      'This wallet is misconfigured. Select the wallet from the main menu on the device to fix it.',
    NO_VALID_WALLET_FOUND:
      'All wallets are misconfigured. Go to the wallets on the main menu and configure them again.',
    NO_WALLET_ON_DEVICE:
      'No wallet found. Create a new wallet on the X1 device first',
    WALLET_IS_LOCKED:
      'Wallet is locked on the device\nUnlock it before continuing with any operations on the wallet',
    ...
    ADD_WALLET_REJECTED:
      'Request rejected on X1 wallet. Restart the process to add a new wallet',
    ...
    ADD_COIN_REJECTED:
      'Request was rejected on the X1 Wallet\nRestart the process to add coins.',
    ADD_COIN_UNKNOWN_ERROR: `Some internal error occured. Disconnect and then reconnect the device and try adding coins.`,
    ...
    CARD_AUTH_UNKNOWN_ERROR: 'Some internal error occured. Retry the process.',

    DEVICE_AUTH_REJECTED: 'Device auth was rejected from the device.',
    DEVICE_AUTH_FAILED:
      'The device seems be compromised. Contact Cypherock support.',
    DEVICE_AUTH_UNKNOWN_ERROR: `Some internal error occured. Disconnect and then reconnect the device and retry.`,
    ...
    DEVICE_UPGRADE_CONNECTION_FAILED_IN_AUTH:
      'Error connecting to the device. Reconnect the device and try again.',
    DEVICE_UPGRADE_KNOWN_ERROR:
      'An internal error occured. Report it to Cypherock immediately.',
    DEVICE_UPGRADE_UNKNOWN_ERROR: `Some internal error occured. Disconnect and then reconnect the device and retry upgrading the device.`,

    LOG_FETCHER_REJECTED:
      'Request rejected from device. Try attaching the device logs again.',
    LOG_FETCHER_DISABLED_ON_DEVICE:
      'Logs are disabled on the device, Enable it from device settings and try again.',
    LOG_FETCHING_CANCEL_FAILED: 'Something went wrong while cancelling.',
    LOG_FETCHER_UNKNOWN_ERROR:
      'Some internal error occurred. Attach the device logs again.',

    RECEIVE_TXN_REJECTED: coin =>
      `Request was rejected on the X1 Wallet\nRestart the process to receive ${coin} on the CySync application`,
    ...
    RECEIVE_TXN_DIFFERENT_ADDRESS_BY_USER: `Contact cypherock if the addresses on the application did not match the addresses on the device`,

    SEND_TXN_REJECTED: (coin: string) =>
      `Request was rejected on the X1 Wallet\nRestart the process to send ${coin} on the CySync application`,
    ...
    SEND_TXN_UNKNOWN_ERROR: `Some internal occured. Disconnect and then reconnect the device and retry.`,

    RECEIVE_TXN_XPUB_MISSING: 'Coin/Xpub is not present on device',
    RECEIVE_TXN_UNKNOWN_ERROR: `Some internal occured. Disconnect and then reconnect the device and retry.`,
