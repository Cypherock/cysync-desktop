...
const BUILD_TYPE_CONFIG = {
    'dev': {
        ...
        "GITHUB_REPO": "Cypherock/Cysync-desktop-test-stm-dev",
    ...
    'debug': {
        ...
        "GITHUB_REPO": "Cypherock/Cysync-desktop-test-stm",
        "SIMULATE_PRODUCTION": false
    },
    'prod': {
        ...
        "LOG_LEVEL": "info",
        "SERVER_ENV": "production",
        "GITHUB_REPO": "Cypherock/Cysync-desktop-beta-stm",
        "SIMULATE_PRODUCTION": false
    },
}

const getBuildType = () => {
    const CMD_ERROR_MSG = 'Invalid command. Expected command: `node <file_name>.js <dev|debug|prod>`'
...
const setConfig = (buildType) => {
    ...
    fs.writeFileSync(configPath, JSON.stringify(BUILD_TYPE_CONFIG[buildType], undefined, 2));
}

const getCommitHash = () => {
    return new Promise((resolve, reject) => {
        childProcess.exec('git log -n 1 --pretty=format:"%H"', (err, stdout, stderr) => {
            if (err || stderr) {
                reject(err || stderr);
                return;
            }
